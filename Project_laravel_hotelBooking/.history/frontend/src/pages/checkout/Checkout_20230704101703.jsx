...
const Checkout = ({ id }) => {
    ...
    const fetch = () => {
        RommServices(id).then(
            res => setServices(res)
        ).catch(e =>console.log("error fetch data room services:",e))
    }
    useEffect(() => {
        if (!services) {
            fetch();
...
// hàm này dùng để kiểm tra thay đổi của credit khi mà người dùng click vào thì chuyển sang trạng thái true và hiện form
