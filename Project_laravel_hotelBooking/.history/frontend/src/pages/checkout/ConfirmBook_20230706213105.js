...
function ConfirmBook() {
    ...
    const fetchServices = () => {
        SelectAllServices().then(
            (services) => {
                services = services.filter(item => item.id in dataBook.services);
                                ...
                                <p className='row'>
                                    {/* lấy services đã chọn bỏ vào đây */}
                                    {
                                        services && services.length ?
                                            services.map(
                                                service => (<div className='col-12 text-secondary'> Hổ trợ quán bar BCS __ 1000$</div>)
                                            )
                                            : ""
                                    }
