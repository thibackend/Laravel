...
function ConfirmBook() {
    ...
    const fetchServices = async (serciesIDs) => {
        // SelectAllServices(serciesIDs).then(
        //     res => {
        //         if (Array.isArray(res)) {
        //             const ser = dataBook.services.map(
        //                 e => services = res.filter(ser => ser.id === e)
        //             )
        //             setServices(ser);
        //         } else {
        ...
        await axios.post("http://localhost:8000/api/serviceGetById",serciesIDs)
        .then(res => console.log('object', object);)
    }
    ...
    const configData = () => {
        ...
        setTotal(dataBook.night * dataBook.price);
    }
    ...
    useEffect(
        () => {
            ...
            if (services) fetchServices(services);
